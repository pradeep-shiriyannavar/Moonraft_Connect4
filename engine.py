...
class GameEngine(object):
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("mooonRaftConnect 4")
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEMOTION:
                    ...
                    if self.turn == self.PLAYER:
                        pygame.draw.circle(self.screen, self.GREEN, (x_pos, int(self.BOX_SIZE / 2)), self.RADIUS)
                    ...
                    if self.turn == self.PLAYER:
                        ...
                        if self.board[self.rows - 1][col] == 0:
                            ...
                            if self.PlayerWin(self.board, self.PLAYER_PIECE):
                                label = self.font.render("You win!!!", 1, self.GREEN)
            ...
            if self.turn == self.AI and not self.game_over:
                ...
                if self.board[self.rows - 1][col] == 0:
                    ...
                    if self.PlayerWin(self.board, self.AI_PIECE):
                        label = self.font.render("Computer Win!!!", 1, self.YELLOW)
                        self.screen.blit(label, (40, 10))
                        self.game_over = True
    ...
    def GamePaint(self, table):
        for c in range(self.cols):
            for r in range(self.rows):
                pygame.draw.rect(table, self.LIGHTBLUE, (c * self.BOX_SIZE, r * self.BOX_SIZE + self.BOX_SIZE, self.BOX_SIZE, self.BOX_SIZE))
        ...
        for c in range(self.cols):
            for r in range(self.rows):
                if self.board[r][c] == self.PLAYER_PIECE:
                    pygame.draw.circle(table, self.GREEN, (
